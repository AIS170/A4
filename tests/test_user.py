...
def test_email_change_success():
    ...
    response = client.put(
        '/user/email',
        data={'new_email': "newemail@newemail.com"},
        follow_redirects=True
    )
    assert response.status_code == 200
...
def test_email_change_new_email_same_as_current_email():
    ...
    response = client.put(
        '/user/email',
        data={'new_email': "user@example.com"},
        follow_redirects=True
    )
    assert response.status_code == 400
    message = json.loads(response.data)
...
def test_password_change_password_too_long():
    ...
    response = client.put(
        '/user/password',
        data={'new_password': "str0ngPassworddd"},
        follow_redirects=True
    )
    assert response.status_code == 400
    message = json.loads(response.data)
